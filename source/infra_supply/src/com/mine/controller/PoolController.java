...
import com.mine.persistence.PoolServiceImpl;
...
public class PoolController {
    protected static final Logger logger = LoggerFactory.getLogger(PoolController.class);
    @ManagedProperty(value = "#{poolService}")
    private PoolServiceImpl poolService;
    private LazyDataModel<PoolEntity> lazyModel;
    private List<Boolean> togglerColumn = new ArrayList<>();
    private PoolEntity newObj;
    private Boolean isEdit = false;
    private List<PoolEntity> selectedObj;
    ...
    public void onStart() {
        ...
        lazyModel = new LazyDataModelBaseNew<>(poolService, filters, orders);
    ...
    public void preInsert() {
        newObj = new PoolEntity();
        isEdit = false;
    }

    public void preUpdate(PoolEntity node) {
        try {
            newObj = node;
            isEdit = true;
        } catch (Exception e) {
    ...
    public void onSaveOrUpdate() {
        ...
        try {
            poolService.saveOrUpdate(newObj);
    ...
    public void delete(PoolEntity delObj) {
        if (delObj != null) {
            try {
                poolService.delete(delObj);
    ...
    public void onDelete() {
        try {
            poolService.delete(selectedObj);
...
//        }
    ...
    public LazyDataModel<PoolEntity> getLazyModel() {
        return lazyModel;
    }

    public void setLazyModel(LazyDataModel<PoolEntity> lazyModel) {
    ...
    public PoolEntity getNewObj() {
        return newObj;
    }

    public void setNewObj(PoolEntity newObj) {
        this.newObj = newObj;
    }
    ...
    public PoolServiceImpl getpoolService() {
        return poolService;
    }

    public void setpoolService(PoolServiceImpl poolService) {
        this.poolService = poolService;
    }

    public List<PoolEntity> getSelectedObj() {
        return selectedObj;
    }

    public void setSelectedObj(List<PoolEntity> selectedObj) {
